...
bool LayoutSaver::restoreLayout(const QByteArray &data, RestoreOptions options)
{
    ...
    for (const LayoutSaver::MainWindow &mw : qAsConst(layout.mainWindows)) {
        ...
        if (!(options & RestoreOption_RelativeToMainWindow))
...
{
    scalingInfo.applyFactors(geometry);
    if (!frame.isNull)
...
void LayoutSaver::DockWidget::scaleSizes()
{
    lastPosition.scaleSizes();
}
...
void LayoutSaver::Anchor::scaleSizes(const ScalingInfo &scalingInfo)
{
    ...
    scalingInfo.applyFactors(pos);

    if (isVertical()) {
        geometry.moveLeft(pos.x());
    } else {
        geometry.moveTop(pos.y());
    }
}
...
{
    scalingInfo.applyFactors(/*by-ref*/geometry);
...
void LayoutSaver::LastPosition::scaleSizes()
{
...
void LayoutSaver::ScalingInfo::applyFactors(QPoint &pt) const
{
...
void LayoutSaver::ScalingInfo::applyFactors(QSize &sz) const
{
    sz.setWidth(qCeil(widthFactor * sz.width()));
    sz.setHeight(qCeil(heightFactor * sz.height()));
}

void LayoutSaver::ScalingInfo::applyFactors(QRect &rect) const
{
    QPoint pos = rect.topLeft();
    QSize size = rect.size();

    applyFactors(/*by-ref*/size);
    applyFactors(/*by-ref*/pos);
